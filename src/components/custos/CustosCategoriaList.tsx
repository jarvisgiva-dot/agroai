...
import { CustoCategoria } from "@/types"

interface CustosCategoriaListProps {
    data: CustoCategoria[]
    selectedIds: number[]
    onSelect: (id: number) => void
    onSelectAll: () => void
    onEdit: (item: CustoCategoria) => void
    onDelete: (id: number) => void
}

export function CustosCategoriaList({
    data,
    selectedIds,
    onSelect,
    onSelectAll,
    onEdit,
    onDelete
}: CustosCategoriaListProps) {
    if (data.length === 0) {
        return <div className="text-center p-8 text-gray-500">Nenhum dado de custo por categoria encontrado.</div>
    }
